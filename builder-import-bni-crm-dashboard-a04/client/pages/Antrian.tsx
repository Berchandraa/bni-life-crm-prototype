...
function PolicyDrawer({ onClose }: { onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-[380px] bg-white shadow-xl flex flex-col h-full">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#EFEFEF]">
          <h2 className="text-base font-semibold text-[#3D3D3D]">Detail Polis</h2>
          <button onClick={onClose} className="p-1 hover:bg-[#FAFAFA] rounded"><IconX /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          <div className="flex items-center gap-3">
            <AvatarPlaceholder size={48}/>
            <div>
              <p className="text-sm font-semibold text-[#464646]">Budi Santoso</p>
              <p className="text-xs text-[#7C7C7C]">Pemegang polis</p>
            </div>
          </div>
          <div className="h-px bg-[#EFEFEF]"/>
          {[
            { label: "No Polis", value: "899210023" },
            { label: "Status", value: "Aktif", badge: true },
            { label: "Produk", value: "BLife Spectra" },
            { label: "Uang Pertanggungan", value: "Rp 250.000.000" },
            { label: "Tanggal Mulai", value: "1 Januari 2020" },
            { label: "Tanggal Berakhir", value: "1 Januari 2030" },
            { label: "Premi Bulanan", value: "Rp 500.000" },
            { label: "Kelas Manfaat", value: "Platinum" },
          ].map((item) => (
            <div key={item.label}>
              <p className="text-xs text-[#7C7C7C] mb-1">{item.label}</p>
              {item.badge ? (
                ...
                <p className="text-sm font-medium text-[#464646]">{item.value}</p>
              )}
            </div>
          ))}
        </div>
        <div className="p-6 border-t border-[#EFEFEF]">
          <button onClick={onClose} className="w-full py-2 rounded-lg bg-[#016986] text-white text-sm font-semibold hover:bg-[#015570] transition-colors">
            Tutup
          </button>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#EFEFEF]">
          <h2 className="text-base font-semibold text-[#3D3D3D]">Buat Tiket Baru</h2>
          <button onClick={onClose} className="p-1 hover:bg-[#FAFAFA] rounded"><IconX /></button>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#464646] mb-1.5">Kategori</label>
          ...
          <div>
            <label className="block text-sm font-medium text-[#464646] mb-1.5">Prioritas</label>
            <div className="relative">
          ...
          <div>
            <label className="block text-sm font-medium text-[#3D3D3D] mb-1.5">Deskripsi</label>
            <textarea value={deskripsi} onChange={e => setDeskripsi(e.target.value)}
              placeholder="Masukan Deskripsi" rows={4}
              className="w-full px-3.5 py-2.5 rounded-lg border border-[#EFEFEF] bg-[#FAFAFA] text-sm text-[#464646] placeholder-[#7C7C7C] focus:outline-none focus:ring-2 focus:ring-[#F47920]/20 resize-none"/>
          </div>
        </div>
        <div className="px-6 pb-6 flex gap-3">
          <button onClick={onClose} className="flex-1 py-2 rounded-lg border border-[#DCDCDC] text-sm font-semibold text-[#464646] hover:bg-[#FAFAFA] transition-colors">
            Batal
          </button>
          <button onClick={onClose} className="flex-1 py-2 rounded-lg bg-[#016986] text-white text-sm font-semibold hover:bg-[#015570] transition-colors">
            Buat Tiket
          </button>
        </div>
      </div>
        ...
        <header className="flex items-center justify-between px-6 sm:px-8 h-[72px] border-b border-[#EFEFEF] gap-4 shrink-0">
          ...
          <div className="flex-1 max-w-md">
            <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-lg border border-[#DCDCDC] bg-[#FAFAFA] shadow-sm">
              ...
              <span className="text-base text-[#7C7C7C]">Search</span>
            </div>
          </div>
        ...
        <div className="flex gap-3.5 px-3.5 pb-3.5 flex-1 min-h-0 overflow-hidden">
          ...
          <div className="w-[321px] shrink-0 flex flex-col gap-3.5 rounded-lg border border-[#EFEFEF] bg-white p-3.5">
            ...
            <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-lg border border-[#DCDCDC] bg-white shadow-sm">
              <IconSearch />
              <span className="text-sm text-[#7C7C7C]">cari nama atau no polish....</span>
            </div>
            {/* Tabs */}
          ...
          <div className="flex-1 min-w-0 flex flex-col rounded-lg border border-[#EFEFEF] bg-white overflow-hidden">
            ...
            <div className="flex items-center justify-between px-3.5 py-2.5 border-b border-[#EFEFEF] shrink-0">
              ...
              <button className="flex items-center gap-2 px-3.5 py-1.5 rounded-md border border-[#EF4444] text-[#EF4444] text-sm font-semibold hover:bg-[#FEF2F2] transition-colors">
                Akhiri Chat
              </button>
            </div>
              ...
              <div className="flex items-center justify-between px-3.5 py-3 bg-white border-t border-[#EFEFEF]">
                <div className="flex items-center gap-6">
                  <Tooltip text="Lampirkan tautan">
                    <button className="hover:opacity-70 transition-opacity"><IconAttachLink /></button>
                  </Tooltip>
                  <Tooltip text="Upload gambar">
                    <button className="hover:opacity-70 transition-opacity"><IconAttachImage /></button>
                  </Tooltip>
                  <Tooltip text="Lampirkan catatan">
                    <button className="hover:opacity-70 transition-opacity"><IconAttachNote /></button>
                  </Tooltip>
                </div>
                <button className="flex items-center gap-2 px-3.5 py-1.5 rounded-md bg-[#F47920] text-white text-sm font-semibold shadow-sm hover:bg-[#D4640F] transition-colors">
                  Kirim <IconSend2 />
                </button>
              </div>
            </div>
